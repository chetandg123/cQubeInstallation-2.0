...
filled_config_file="../../ConfigFiles/Fill_workflow_upgradation_config_file.yml"
...
test_result_file="../../TestResult/upgradeation_status_file.txt"
s3_access_key=$(awk ''/^s3_access_key:' /{ if ($2 !~ /#.*/) {print $2}}' ./ConfigFiles/workflow_upgradation_testing_config.yml)
s3_secret_key=$(awk ''/^s3_secret_key:' /{ if ($2 !~ /#.*/) {print $2}}' ./ConfigFiles/workflow_upgradation_testing_config.yml)
git_branch=$(awk ''/^git_branch:' /{ if ($2 !~ /#.*/) {print $2}}' ./ConfigFiles/workflow_upgradation_testing_config.yml)
...
echo "${txtblue}Test Case:2********Checking error messages without filling the base_dir path in upgradation_config.yml testing is started**********""${txtrst}" >> "$test_result_file"
remove_config_file
copy_new_config_file
sudo sed -i 's/diksha_columns: false /diksha_columns:/g' "$config_file"
sudo sed -i 's/session_timeout: 7D /session_timeout:/g' "$config_file"
...
echo "********Checking error messages without only filling the base_dir path in upgradation_config.yml testing is started**********" >> "$test_result_file"
...
copy_filled_config_file
printf 'yes\n' | sudo ./upgradation_validate.sh | tee "$actual_output_file"
output=$(grep -c "cQube Workflow upgraded successfully!!" $actual_output_file)
if [ $output = 1 ]
then
  echo "${txtgreen}cQube workflow upgraded successfully""${txtrst}" >> "$test_result_file"
else
  echo "${txtred}cQube workflow is not upgraded successfully""${txtrst}" >> "$test_result_file"
fi
echo "********Filling the valid parameters in the upgradation_config.yml file testing is completed****************" >> "$test_result_file"
